...
def save_to_file():
    ...
    with open(file_name, "r") as file:
        if site in file.read():
            password_label.config(text="the site already saved in the file!")
        else:
            with open(file_name, "a") as file:
                file.write(f"site_address :{site} ")
                file.write(f"password :{generated_pass} \n")
            password_label.config(text="password saved to the file!")
...
retive_pass_button = Button(window, text="Retrieve Password", fg="white", bg=blue, width=12, font=(font_name, 10))
