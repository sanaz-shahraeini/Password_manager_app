...
def recovery_password():
    ...
    exist_rows = df.loc[df.isin([site]).any(axis=1)].values.tolist()
    if exist_rows:
        password_label.config(text=exist_rows[0][1], fg=blue)
...
def save_to_gsheet():
    ...
    if site not in out:
        ...
        messagebox.showwarning("Warning", "site already saved in the google sheet!")
        site_address_entry.delete(0, END)
